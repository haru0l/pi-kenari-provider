...
function requireKey(apiKey, what = "this endpoint") {
    if (!apiKey)
        throw new Error(`kenari API key required for ${what}`);
    return apiKey;
}
...
export async function generateAudio(options) {
    ...
    const response = await postJson(`${BASE_URL_KENARI}/audio/speech`, apiKey, body, options.signal);
    if (!response.ok) {
...
export async function generateMusic(options) {
    ...
    if (options.instrumental !== true && !options.lyrics) {
        throw new Error("kenari music generation requires lyrics (or instrumental: true with prompt)");
    }
    ...
    const response = await postJson(`${BASE_URL_KENARI}/music/generations`, apiKey, body, options.signal);
    ...
    if (json.error) {
        ...
        throw new Error(`kenari music generation failed: ${json.error.code ?? ""} ${json.error.message ?? ""}`.trim());
    }
    if (!response.ok) {
        throw new Error(`kenari music generation failed (${response.status}): ${JSON.stringify(json)}`);
    }
    const b64 = json.data?.[0]?.b64_json;
    if (!b64)
        throw new Error("kenari music generation returned no audio data");
    const bin = atob(b64);
...
export async function createVideoJob(options) {
    ...
    const response = await postJson(`${BASE_URL_KENARI}/videos/generations`, apiKey, body, options.signal);
    if (!response.ok) {
        throw new Error(`kenari video creation failed (${response.status}): ${await response.text()}`);
    }
    return (await response.json());
}
/** Poll a video job until it is done, failed, or expired. */
export async function pollVideoJob(jobId, options) {
    const apiKey = requireKey(options.apiKey, "video generation");
    ...
    for (;;) {
        ...
        if (!response.ok) {
            throw new Error(`kenari video poll failed (${response.status}): ${await response.text()}`);
        }
...
export async function generateVideo(options) {
    ...
    if (done.status !== "done" || !done.url) {
        throw new Error(`kenari video job ${job.id} finished with status "${done.status}"`);
    }
