...
export async function generateAudio(options) {
    ...
    if (!apiKey)
        throw new Error("kenari API key required for audio generation");
    const body = {
...
export async function generateMusic(options) {
    const apiKey = options.apiKey;
    if (!apiKey)
        throw new Error("kenari API key required for music generation");
    const body = {
        prompt: options.prompt,
        model: options.model,
    };
    ...
    if (options.response_format !== undefined)
        body.response_format = options.response_format ?? "mp3";
...
export async function generateVideo(options) {
    const apiKey = options.apiKey;
    if (!apiKey)
        throw new Error("kenari API key required for video generation");
    const body = {
        prompt: options.prompt,
        model: options.model,
    };
